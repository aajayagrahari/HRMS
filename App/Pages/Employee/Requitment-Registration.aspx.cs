...
public partial class Pages_Admin_Requitment_Registration : System.Web.UI.Page
{
    ...
    protected void btn_add_Click(object sender, EventArgs e)
    {
        ...
        if (ddd_education.SelectedIndex == 0)
        {
    ...
    {
        if (ddl_proEducation.SelectedIndex ==0)
        {
            lbl_msg.Text = "Please Enter Professional Qualification !.";
            ddl_proEducation.Focus();
            return;
        }
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (gv_pro_education.Rows.Count == 0 && gv_education.Rows.Count == 0)
            {
                //if (gv_education.Rows.Count == 0)
                //{
                    //lbl_msg.Text = " Please Enter Qualification .";
                lbl_msg.Text = " Please Enter either Qualification or professional qualification detail .";
                    gv_education.Focus();
                    return;
               // }
            ...
            foreach (ErrorHandlerClass err in objRequitmentRegistrationMasterManager.SaveRequitmentRegistrationMaster(ObjRequitmentRegistrationMaster, SetEducationMasterObjInfo(), SetProfesionalEducationMasterObjInfo(), SetExperienceMasterObjInfo(), SetReferencesMasterObjInfo(), SetLanguageMasterObjInfo()))
            {
                ...
                else if (err.Type == "S")
                {
                    ...
                    if (ReturnValue == "")
                    {
                        lbl_msg.Text = "You Have Allready Appllied fot this Advertisement !.";
                        return;
                    }
                    else
                    {

                        Boolean SendMail = sendMail(txt_email.Text, ReturnValue);
                        if (SendMail == true)
                        {
                        ...
                        BindWorkExperience();
                        ClearAll();
                        Response.RedirectPermanent("~/Pages/Employee/frmPrintRegistration.aspx?rid=" + ReturnValue);
                    }
                    //StringBuilder sb = new StringBuilder();
                    //sb.Append("<script language='javascript'>");
                    //sb.Append("window.open('../Universal/frmPrintRegistration.aspx?rid=" + ReturnValue + "','CustomPop','height=500,width=900');");
                    //sb.Append("</script>");
                    //Type t = this.GetType();
    ...
    void PostMaster()
    {
        ...
        ds = ObjPostMasterManager.GetPostMaster4Registration();
    ...
    public DataTable CreateEducation()
    {
        try
        {
            ...
            dr["Qualification"] = ddd_education.SelectedItem.Text;
    ...
    public DataTable CreateProEducation()
    {
        try
        {
            ...
            DataRow dr = dtItems.NewRow();
            dr["Qualification"] = ddl_proEducation.SelectedItem.Text;
    ...
    void ClearProfessionalField()
    {
        ...
        ddl_proEducation.SelectedIndex = 0;
        txt_pro_school.Text = "";
    ...
    public bool sendMail(string strEmailId, string registrationnId)
    {
        try
        {
            ...
            Msg.Body = "Hi, <br/>Please check your Login Detailss<br/><br/>Registration Id: " + registrationnId +  "<br/><br/>";
    ...
    public void SetRequitmentRegistrationMasterObjInfo()
    {
        ...
        ObjRequitmentRegistrationMaster.PhoneNo = txt_phone.Text;
    ...
    public ICollection<EducationMaster>SetEducationMasterObjInfo()
    {
        ...
        EducationMaster ObjEducationMaster = null;
        if (gv_education.Rows.Count > 0)
        {
            for (int i = 0; i < gv_education.Rows.Count; i++)
            {
                 ObjEducationMaster = new EducationMaster();
    ...
    public ICollection<ProfessionalEducationMaster> SetProfesionalEducationMasterObjInfo()
    {
        ...
        ProfessionalEducationMaster ObjProfessionalEducationMaster = null;
        if (gv_pro_education.Rows.Count > 0)
        {
            for (int i = 0; i < gv_pro_education.Rows.Count; i++)
            {
                ObjProfessionalEducationMaster = new ProfessionalEducationMaster();
    ...
    public ICollection<ReferencesMaster> SetReferencesMasterObjInfo()
    {
        ...
        ReferencesMaster objReferencesMaster = null;
       
        if (gv_references.Rows.Count > 0)
        {
            for (int i = 0; i < gv_references.Rows.Count; i++)
            {
                objReferencesMaster = new ReferencesMaster();
    ...
    protected void ddl_post_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddl_post.SelectedIndex > 0)
        {
            ...
            DataSet ds = new DataSet();
            ds = ObjPostMasterManager.GetPostMaster4Id(post_id);
            if (ds.Tables[0].Rows.Count > 0)
            {
                lbl_max_age.Value = ds.Tables[0].Rows[0]["MaxAge"].ToString();
                lbl_min_Age.Value = ds.Tables[0].Rows[0]["MinAge"].ToString();
                ViewState["AdvertisementId"] = ds.Tables[0].Rows[0]["AdvertisementId"].ToString();
                BindEducation(post_id);
            }
            DataTable dtEducation = objRequitmentRegistrationMasterManager.getQualificationDetail(Convert.ToInt32(ddl_post.SelectedValue));
            DataTable dtProffestional = objRequitmentRegistrationMasterManager.getProffestionalDetail(Convert.ToInt32(ddl_post.SelectedValue));
            DataTable dtWorkExperience = objRequitmentRegistrationMasterManager.getWorkExperienceDetail(Convert.ToInt32(ddl_post.SelectedValue));

            if (dtEducation.Rows.Count == 0)
            {
                rowEduQualification1.Visible = false;
                rowEduQualification2.Visible = false;
                rowEduQualification3.Visible = false;
            }
            else
            {
                rowEduQualification1.Visible = true;
                rowEduQualification2.Visible = true;
                rowEduQualification3.Visible = true;
            }
            if (dtProffestional.Rows.Count == 0)
            {
                rowProQualification1.Visible = false;
                rowProQualification2.Visible = false;
                rowProQualification3.Visible = false;
            }
            else
            {
                rowProQualification1.Visible = true;
                rowProQualification2.Visible = true;
                rowProQualification3.Visible = true;
            }
            //if (dtWorkExperience.Rows.Count == 0)
            //{
            //    rowWorkWxperience1.Visible = false;
            //    rowWorkWxperience2.Visible = false;
            //    rowWorkWxperience3.Visible = false;
            //}
            //else
            //{
            //    rowWorkWxperience1.Visible = true;
            //    rowWorkWxperience2.Visible = true;
            //    rowWorkWxperience3.Visible = true;
            //}
            ddd_education.DataSource = dtEducation;
            ddd_education.DataValueField = dtEducation.Columns["val"].ToString();
            ddd_education.DataTextField = dtEducation.Columns["txt"].ToString();
            ddd_education.DataBind();
            ddd_education.Items.Insert(0, "Select");

            ddl_proEducation.DataSource = dtProffestional;
            ddl_proEducation.DataValueField = dtEducation.Columns["val"].ToString();
            ddl_proEducation.DataTextField = dtEducation.Columns["txt"].ToString();
            ddl_proEducation.DataBind();
            ddl_proEducation.Items.Insert(0, "Select");
            rowWorkExp.Visible = true;
            lblWorkExperience.Text = dtWorkExperience.Rows[0]["ExperienceDetail"].ToString();
    ...
    void BindEducation(int PostId)
    {
        DataSet ds = new DataSet();
        ds = ObjPostMasterManager.GetPostEducation4Id(PostId);
        if (ds.Tables[0].Rows.Count > 0)
        {

            ddd_education.DataSource = ds.Tables[0];
            ddd_education.DataTextField = "QualificationDetail";
            ddd_education.DataValueField = "QualificationDetail";
            ddd_education.DataBind();
            ddd_education.Items.Insert(0, "-- Select --");
        }
        if (ds.Tables[1].Rows.Count > 0)
        {
            ...
            ddl_proEducation.DataSource = ds.Tables[1];
            ddl_proEducation.DataTextField = "QualificationDetail";
            ddl_proEducation.DataValueField = "QualificationDetail";
            ddl_proEducation.DataBind();
            ddl_proEducation.Items.Insert(0, "-- Select --");
        }
    }
